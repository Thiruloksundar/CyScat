"""
generate_s_matrix_1layer_dielectric.jl
=======================================
Compute the S-matrix for a single layer of 10 dielectric cylinders (n=1.3).
...
Usage:
    julia generate_s_matrix_1layer_dielectric.jl
    julia generate_s_matrix_1layer_dielectric.jl --num_cyl 20
"""
...
const PERIOD     = 12.81
const RADIUS     = 0.25
const N_CYL_REF  = 1.3          # dielectric refractive index
const MU         = 1.0
const CMMAX      = 5
...
# ── Plot geometry ────────────────────────────────────────────────────────────
function plot_geometry(clocs, thickness; outfile="dielectric_geometry")
    num_cyl = size(clocs, 1)
    p = plot(aspect_ratio=:equal, size=(700, 400),
             xlabel="x", ylabel="y",
             title="$num_cyl Dielectric (n=$(N_CYL_REF)) cylinders  (λ=$(WAVELENGTH), Λ=$(PERIOD))")
...
function plot_singular_values(S, nmax, n_eva, num_cyl; outfile="dielectric_singular_values")
    nm = 2*nmax + 1
    S21 = S[nm+1:end, 1:nm]
    S21T = n_eva > 0 ? S21[n_eva+1:end-n_eva, n_eva+1:end-n_eva] : S21
    tau = svdvals(S21T)
    p = bar(1:length(tau), tau .^2,
            xlabel="Channel index", ylabel="τ²",
            title="Transmission eigenvalues — $num_cyl Dielectric (n=$(N_CYL_REF))",
...
function build_fields(S, nmax, n_eva, thickness, mode,
    ...
    if n_eva > 0; Input[n_eva+1:end-n_eva] = v_in; else Input = copy(v_in); end
...
function main()
    ...
    println("=" ^ 65)
    println("  CyScat — Single-Layer Dielectric (n=$(N_CYL_REF)) Example")
    println("=" ^ 65)
    @printf("  λ=%.2f  Λ=%.2f  r=%.2f  N_cyl=%d  seed=%d\n",
            WAVELENGTH, PERIOD, RADIUS, num_cyl, seed)
    @printf("  nmax=%d  (N_prop=%d  N_eva=%d)  thickness=%.1f\n",
            nmax, n_prop, n_eva, thickness)
    ...
    cepmus = repeat([N_CYL_REF^2 MU], num_cyl, 1)
    crads  = fill(RADIUS, num_cyl)

    # 1. Geometry
    println("\n  [1] Drawing geometry...")
    plot_geometry(clocs, thickness; outfile=joinpath(OUTDIR, "dielectric_geometry"))
    ...
    plot_singular_values(S, nmax, n_eva, num_cyl; outfile=joinpath(OUTDIR, "dielectric_singular_values"))
    ...
    for mode in ["normal", "opt_trans"]
        ...
        t0 = time()
        F, xg, yf, _, lbl = build_fields(S, nmax, n_eva, thickness, mode,
    ...
    make_movie(fields_list, x_grid, y_full, thickness, clocs, crads,
               WAVELENGTH, joinpath(OUTDIR, "dielectric_wavefield"); n_frames=frames, fps=fps)
