...
def _build_spectral_params_jax(lambda_wave, period, phiinc, jmax):
    ...
    # Angles with MATLAB-compatible branch cut:
    # Python arcsin gives +i*acosh for arg > 1; MATLAB gives -i*acosh.
    # Conjugate positive evanescent entries using the sign_fix pattern.
    arg = kxs / k0
    arg_c = arg + 0j
    angles_raw = jnp.arcsin(arg_c)
    sign_fix = 1 - 2 * (jnp.real(arg) > 1).astype(jnp.float64)
    angles = jnp.real(angles_raw) + 1j * (jnp.imag(angles_raw) * sign_fix)
