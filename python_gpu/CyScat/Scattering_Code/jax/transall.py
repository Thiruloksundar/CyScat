...
def _compute_self_sum_jax(max_order, sp, period, phiinc):
    """Self-sum via JAX spatial path (ob=so=[0,0]), differentiable w.r.t. sp['k0']."""
    cms_int = jnp.arange(-2 * max_order, 2 * max_order + 1, dtype=jnp.int32)
    ob = jnp.array([[0.0, 0.0]])
    so = jnp.array([[0.0, 0.0]])
    result = _spatial_sum_jax(ob, so, cms_int, sp, period, phiinc)
    return result[0]  # squeeze batch dimension
