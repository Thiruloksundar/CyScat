...
def transall(clocs, cmmaxs, period, lambda_wave, phiinc, sp, total_steps, clocs_concrete=None):
    ...
    # ── Self-sum ───────────────────────────────────────────────────────
    # Use fast numpy path by default; JAX path only when λ is being traced
    # (inside jax.jvp/grad) so the self-sum carries gradients through k0.
    _tracing_lambda = isinstance(sp.get('k0'), jax.core.Tracer)
    if _tracing_lambda:
        self_sum_vector = _compute_self_sum_jax(max_order, sp, period, phiinc)
    else:
        self_sum_vector = _compute_self_sum_numpy(
            max_order, period, lambda_wave, phiinc, sp)
...
@jax.custom_jvp
def _self_sum_numpy_traced(k0_traced, max_order, period, phiinc, sp_concrete):
    """
    Forward: call numpy self-sum at the given k0 with the non-k fields of
    sp_concrete reused. Returns a complex (4*max_order+1,) array.

    Only k0 carries tangents; sp_concrete fields are non-λ-dependent for
    the self-sum (which uses the spatial path: only sp['k0'] is read).
    """
    k = float(k0_traced)
    sp_use = dict(sp_concrete)
    sp_use['k0'] = k
    lam = 2 * np.pi / k
    return _compute_self_sum_numpy(max_order, period, lam, phiinc, sp_use).astype(np.complex128)


@_self_sum_numpy_traced.defjvp
def _self_sum_numpy_traced_jvp(primals, tangents):
    k0_p, max_order, period, phiinc, sp_concrete = primals
    k0_t, _, _, _, _ = tangents
    k0_f = float(k0_p)
    # Central FD on k0.
    h = max(1e-6, abs(k0_f) * 1e-8)
    sp_p = dict(sp_concrete); sp_p['k0'] = k0_f + h
    sp_m = dict(sp_concrete); sp_m['k0'] = k0_f - h
    vp = _compute_self_sum_numpy(max_order, period, 2*np.pi/(k0_f+h), phiinc, sp_p)
    vm = _compute_self_sum_numpy(max_order, period, 2*np.pi/(k0_f-h), phiinc, sp_m)
    dvdk = ((vp - vm) / (2 * h)).astype(np.complex128)
    primal_out = _self_sum_numpy_traced(k0_p, max_order, period, phiinc, sp_concrete)
    return primal_out, dvdk * k0_t


def _compute_self_sum_jax(max_order, sp, period, phiinc):
    """
    Self-sum carrying a correct λ-derivative.

    The previous _spatial_sum_jax implementation mismatched numpy's
    Shanks-with-isfinite-revert behaviour (numpy's `valid = isfinite(...)`
    falls back to a1[idx] whereas JAX's safe-denom always keeps the
    candidate). That produced a ~14 % forward error and ~10× derivative
    error vs numpy/Julia. We instead reuse the numpy forward and supply an
    FD-based custom JVP so AD propagates a correct k-derivative.
    """
    # Build concrete (non-traced) snapshot of sp for numpy use.
    sp_concrete = {kk: (np.asarray(vv) if hasattr(vv, 'shape')
                        and not isinstance(vv, jax.core.Tracer) else vv)
                   for kk, vv in sp.items()}
    # Drop any tracer entries (only k0 should be traced here).
    sp_concrete.pop('k0', None)
    return _self_sum_numpy_traced(sp['k0'], max_order, period, phiinc, sp_concrete)
