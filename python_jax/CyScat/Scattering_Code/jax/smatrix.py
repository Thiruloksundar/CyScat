...
def smatrix(clocs, cmmaxs, cepmus, crads, period, lambda_wave, nmax, d, sp, interaction,
    ...
    phiinc = sp['phiinc']
    jmax_val = int(sp['jmax'])

    # === Conditionally build JAX-traced spectral parameters ===
    # Only needed when lambda_wave is a JAX tracer (inside jax.jvp/grad).
    # Normal calls use the fast numpy sp dict as-is.
    _tracing_lambda = isinstance(lambda_wave, jax.core.Tracer)
    if _tracing_lambda:
        k0_jax, kxs_jax, kys_jax, angles_jax = _build_spectral_params_jax(
            lambda_wave, period, phiinc, jmax_val)
        k = k0_jax
        sp_jax = dict(sp)
        sp_jax['k0'] = k0_jax
        sp_jax['kxs'] = kxs_jax
        sp_jax['kys'] = kys_jax
        sp_jax['Angles'] = angles_jax
    else:
        sp_jax = sp
    ...
    if interaction == 'On':
        t = transall(clocs, cmmaxs_np, period, lambda_wave, phiinc, sp_jax, total_steps,
                     clocs_concrete=clocs_concrete)
    else:
        t = transall(clocs, cmmaxs_np, -1, lambda_wave, phiinc, sp_jax, total_steps,
                     clocs_concrete=clocs_concrete)
    ...
    kxs_arr = jnp.asarray(sp_jax['kxs']).flatten()
    ...
    W1_up, W2_up = _build_projection_matrices(clocs, cmmaxs_np, nmax, d, sp_jax, up_down=1)

    s11matrix = W1_up @ C_up
    s21matrix = W2_up @ C_up

    # Add incident field transmission: S21[nmax+nin, nin_idx] += exp(-i*kyex*d)
    kys_arr = jnp.asarray(sp_jax['kys']).flatten()
    ...
    W1_down, W2_down = _build_projection_matrices(
        clocs, cmmaxs_np, nmax, d, sp_jax, up_down=-1, y_shift=-d)
...
# ─────────────────────────────────────────────────────────────────────────────
# JAX spectral parameter builder (for λ-differentiability)
# ─────────────────────────────────────────────────────────────────────────────

def _build_spectral_params_jax(lambda_wave, period, phiinc, jmax):
    """
    Compute λ-dependent spectral parameters using JAX ops for autodiff.

    Returns k0, kxs (column vector), kys, Angles — all JAX-traced so that
    gradients w.r.t. lambda_wave propagate through the full smatrix pipeline.
    """
    k0 = 2 * jnp.pi / lambda_wave

    m_indices = jnp.arange(-jmax, jmax + 1, dtype=jnp.float64)
    if phiinc == np.pi / 2:  # normal incidence — cos(π/2) = 0
        kxs = (2 * jnp.pi * m_indices / period).reshape(-1, 1)
    else:
        kxs = (2 * jnp.pi * m_indices / period + k0 * jnp.cos(phiinc)).reshape(-1, 1)

    # ky with correct evanescent decay (same as ky.py but inline for clarity)
    kys = ky(k0, kxs.astype(jnp.complex128))

    # Angles with MATLAB-compatible branch cut, constructed holomorphically
    # per-branch so JAX derivatives are correct (the earlier real/imag+sign_fix
    # form gave the right forward value but a wrong-sign derivative on
    # evanescent modes, because real/imag split is non-holomorphic).
    #
    #   |arg| ≤ 1  : angles = arcsin(arg)          (real)
    #   arg  > 1   : angles = π/2  − i·acosh( arg) (MATLAB convention)
    #   arg  < −1  : angles = −π/2 + i·acosh(−arg)
    arg = kxs / k0
    prop     = jnp.abs(arg) <= 1
    pos_evan = jnp.real(arg) > 1
    # Safe inputs (avoid NaN in untaken branches and in their gradients).
    arg_prop_safe = jnp.where(prop, arg, 0.0)
    arg_acosh_safe = jnp.where(prop, 2.0, jnp.abs(arg))    # ≥ 1 where evanescent
    asin_real = jnp.arcsin(arg_prop_safe)                   # real part when prop
    acosh_val = jnp.arccosh(arg_acosh_safe)                 # real, ≥ 0
    real_part = jnp.where(prop, asin_real,
                          jnp.where(pos_evan, jnp.pi / 2, -jnp.pi / 2))
    imag_part = jnp.where(prop, 0.0,
                          jnp.where(pos_evan, -acosh_val, acosh_val))
    angles = real_part + 1j * imag_part
    angles = angles.reshape(kxs.shape)

    return k0, kxs, kys, angles
